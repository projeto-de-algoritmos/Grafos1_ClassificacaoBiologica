...
def enumerate_data():
    ...
    for x in range(len(data_set['species'])):
        for groups in ('species', 'genus', 'family', 'order', 'class', 'phylum', 'kingdom'):
            if (data_set[groups][x] not in list(data_enumerated.values())):
                data_enumerated[cont] = data_set[groups][x]
                cont += 1

    with open('de.json', 'w') as json_file:
        json.dump(data_enumerated, json_file)
    return data_enumerated


data_enumerated = enumerate_data()
...
def get_id_name(id=None, name=None):
    ...
    if name:
        if not name in list(data.values()):
            return -1
        return str(list(data.values()).index(name)+1)


def enumerate_new_data(data_enumerated, species):
    cont = len(data_enumerated)+1
    for x in species:
        if-1 == get_id_name(name=x):
            data_enumerated[str(cont)] = x
            cont += 1

    return data_enumerated


def read_enumerated_data():
    if not os.path.exists('de.json'):
        enumerate_data()
    with open('de.json', 'r') as json_file:
        data_enumerated = json.load(json_file)
    return (data_enumerated)
    ...
    for x in species:
        if index < len(species):
            ...
            if not (get_id_name(name=species[-1]) in adjacency_list.get('0')):
                adjacency_list['0'].append(get_id_name(name=species[-1]))
...
def register_from_this(register, rest):
    ...
    while rest:
        if get_id_name(id=rest[-1]):
            ...
            rest.pop()
    write_db(register)
    data_enumerated = read_enumerated_data()
    data_enumerated = enumerate_new_data(data_enumerated, register)
